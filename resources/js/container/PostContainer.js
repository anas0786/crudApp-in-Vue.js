...
class PostContainer {
    ...
    async store(description, title) {
        ...
        try {
            var response = await axios.post('/api/store', {
                title: title,
    ...
    async update(description, title, post_id) {
        var errors = {}

        try {
            var response = await axios.post('/api/update/' + post_id, {
                title: title,
                description: description,
                user_id: sessionStorage.getItem("user_id"),
            }, {
            ...
            if (response.data.message == "successfully data update") {
